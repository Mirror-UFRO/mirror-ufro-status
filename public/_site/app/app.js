(function(){
    ...
    function MainController($rootScope, $scope, $http) {
        ...
        $scope.load = function() {
            ...
            $http.get('https://mirror.ufro.cl/status').then(function(response) {
                var data = response.data;

                // Check settings available for each mirror
                for (var repo in data.mirrors) {
                    // Ignore invalid keys or repos without available config
                    //if (!data.mirrors.hasOwnProperty(repo) || !repoconfig.hasOwnProperty(repo)) {
                    ...
                    for (var j in repoconfig[repo]) {
                        ...
                        if (j === 'logo') {
                            ...
                            val = val.replace('@', '_site/img/logos');
                        }
                    ...
                    if (data.mirrors[repo].hasOwnProperty('status')) {
                        ...
                        if (typeof r.status === 'number') {
                            r.status = 'in progress (' + (r.status * 100).toFixed(2) + '%' + ')';
